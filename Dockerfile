...
RUN R -e "install.packages(c('tidyverse', 'lme4', 'brms', 'BH', 'StanHeaders'), repos = 'http://cran.us.r-project.org', Ncpus = parallel::detectCores())"
RUN R -e "packageurl <- 'https://cran.r-project.org/src/contrib/rstan_2.21.3.tar.gz'; install.packages(packageurl, repos = NULL, type = 'source', dependencies = TRUE, Ncpus = parallel::detectCores())"
